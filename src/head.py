    ...
    if min_interval < 5000:
        while x_coord < result.shape[1]:
            if np.amax(result[:,x_coord]) > 0.1:
                ...
                coordinate = np.append(coordinate, [[x_coord],[y_coord]], axis = 1)
...
def template_finding(img, res, bar_colors):
    ...
    cv2.destroyAllWindows()
    '''
    colorArea = np.where(hsv[:,:,0] == colorValue_H, hsv[:,:,0], 0) # 진환이가 만들어준 남은 이미지들로 바꾸기
    '''
    ...
    template = res[np.amin(template_coord[:,1]) : np.amax(template_coord[:,1]) + 1, np.amin(template_coord[:,0]): np.amax(template_coord[:,0]) + 1, :]
...
if __name__ == '__main__':
    ...
    for i in range(number_colors):
        ...
        _, _, min_interval = template_matching(img, result[i], template, min_interval)
        

    for i in range(number_colors):
        template = cv2.imread(filename+"/template_%i.png" %i)
        matched_image, coord, min_interval = template_matching(img, result[i], template, min_interval)
